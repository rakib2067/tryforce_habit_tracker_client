  ...
  habits.forEach((habit) => {
    ...
    incrementor.addEventListener("click", async () => {
      try {
        ...
        let resp = await updateHabitTimesDone(
          habit.id,
          "increment",
        ...
        if (resp.completed == true) {
          ...
          overallProgress.setAttribute(
            "data-done",
            Math.floor((resp.dayscompleted / (resp.daysexist + 1)) * 100)
          );
    ...
    decrementor.addEventListener("click", async () => {
      try {
        ...
        if (habit.getAttribute("completed") == "true") {
          ...
          console.log(resp.dayscompleted, resp.daysexist);
          overallProgress.setAttribute(
            "data-done",
            Math.floor((resp.dayscompleted / (resp.daysexist + 1)) * 100)
          );
    ...
    function renderOverall() {
      let percent = overallProgress.getAttribute("data-done");
